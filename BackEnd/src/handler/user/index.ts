...
import { getAllUser } from './get-all';
...
export const _handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    if (event.body === null) {
    ...
    switch (request.getMethod()) {
        case "PostParam":
            rel = await postUser(accountId, request.getParam());
            return createResponse(200, jsonRpcResponseEncoder.createJsonStr());
        case "Delete":
            rel = await deleteUsers(accountId, request.getParam());
            return createResponse(200, jsonRpcResponseEncoder.createJsonStr());
        case "GetAll":
            rel = await getAllUser(accountId);
